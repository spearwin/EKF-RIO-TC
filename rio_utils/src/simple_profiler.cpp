...
SimpleProfiler::SimpleProfiler(const bool is_on)
    : kPrefix{"[SimpleProfiler]: "}, is_on_{is_on}, next_id_{0} {}

void SimpleProfiler::start(const std::string& key) {
  if (is_on_) start_times_[key] = std::chrono::system_clock::now();
}

float SimpleProfiler::stopWithRuntimeMs(const std::string& key) {
  if (stop(key)) return profile_data_[key].execution_ms.back();
  return 0.0;
}

bool SimpleProfiler::stop(const std::string& key) {
  if (!is_on_) return false;
...
RuntimeStatistics SimpleProfiler::getStatistics(const std::string& key) {
  if (!is_on_) return RuntimeStatistics();
...
std::string SimpleProfiler::toString(const uint indent) {
  ...
  for (const auto& elem : profile_data_) {
    ...
    ss << indent_str << elem.first << ": "
       << elem.second.statistics.toStringMs() << "\n";
  ...
  // overall
  ss << std::string(indent, ' ') << "total" << ": " << overall.toStringMs()
     << "\n";
...
std::string SimpleProfiler::toMarkdownTable() {
  ...
  for (const auto& elem : profile_data_) {
    std::stringstream ss;
    calculateProfileStatistics(elem.first);
    ss << "|" << elem.first << "|" << elem.second.statistics.mean_ms << "|"
       << elem.second.statistics.max_ms << "|" << elem.second.statistics.min_ms
       << "|\n";
    str_tmp.at(elem.second.id) = ss.str();
  ...
  // overall
  ss << "|total|" << overall.mean_ms << "|" << overall.max_ms << "|"
     << overall.min_ms << "|\n";

  return ss.str();
}

RuntimeStatistics SimpleProfiler::calculateProfileStatistics(
  ...
  if (data == profile_data_.end()) {
    ...
    data->second.statistics.max_ms = *std::max_element(
        data->second.execution_ms.begin(), data->second.execution_ms.end());
    data->second.statistics.min_ms = *std::min_element(
        data->second.execution_ms.begin(), data->second.execution_ms.end());
    data->second.statistics.mean_ms =
        std::accumulate(data->second.execution_ms.begin(),
                        data->second.execution_ms.end(), 0.0f) /
